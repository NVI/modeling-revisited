// polmod.cpp : Defines the entry point for the console application.
//
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	filenames.push_back("file1.csv");
	filenames.push_back("file2.csv");
	filenames.push_back("file3.csv");
	filenames.push_back("file4.csv");
    ...
    Data data(filenames, setup, eduskunta, bundle);

    std::cout << "Initialization done" << std::endl;

    // input
    // data.inputPolitics(); // optional
    // data.inputNetwork(); // optional

    // algorithm
    for (int iteration = 0; iteration < setup.getIterations(); ++iteration) {
        data.iterateBundle(prng, rfloat);
