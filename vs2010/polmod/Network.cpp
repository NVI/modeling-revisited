...
void Network::cluster(Setup& setup, System& system, boost::random::mt19937& prng, boost::uniform_01<>& rfloat) {
    ...
    while (!not_in_cluster.empty()) {
        ...
        while (!potential_expanders.empty()) {
            ...
            for (std::set<int>::iterator it = work_copy.begin(); it != work_copy.end(); ++it) {
                int candidate = *it;
                bool conditions [3];
                conditions[0] = (current_cluster.count(candidate) == 0); // not yet in current cluster
                conditions[1] = (friends[socialite].count(candidate) == 1); // they are friends
    ...
    for (unsigned int i = 0; i < list_of_clusters.size(); ++i) {
